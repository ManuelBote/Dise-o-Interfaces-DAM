...
function agregarTareaKanban() {
    ...
    if (titulo === '' ||             responsable === '') {
        alert('Por favor escribe una tarea y/o responsable');
        return;
    }
...
function renderizarTarea(tarea, estado) {
    ...
    // HTML de la tarjeta
    tarjeta.innerHTML = `
        <div id="tarea${tarea.id}" class="tarjeta-titulo">${tarea.titulo}</div>
        <div class="tarjeta-titulo">Responsable: ${tarea.responsable}</div>
        ...
        <div class="tarjeta-acciones">
            ...
            <button class="btn-accion btn-modificar" onclick="modificarTitulo('tarea${tarea.id}')">Mod titulo</button>
        </div>
    `;
...
function moverTarea(id, estadoActual, direccion) {
    ...
    // Actualizar contadores de ambos estados
    actualizarContador(estadoActual);
...
function modificarTitulo(id){
    const divTitulo = document.getElementById(id);
    const texto = prompt("Escribe el nuevo titulo");
    divTitulo.innerHTML = `${texto}`;
}